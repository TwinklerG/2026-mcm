...
class AdaptiveMCMCSampler:
    ...
    def sample_season(
        self, season: int, return_samples: bool = False, use_cache: bool = True
        ...
        if season not in self.seasons_data:
            return {}, MCMCDiagnostics(0, 0, 0, 0, []), None
        ...
        if use_cache:
            ...
            if cached is not None:
                ...
                if not return_samples:
                    samples = None
                return posterior_mean, diagnostics, samples
        ...
        if return_samples:
            return posterior_mean, diagnostics, samples
        return posterior_mean, diagnostics, None
    ...
    def sample_season_with_ess_check(
        ...
        """
        # 首次采样
        posterior, diagnostics, samples = self.sample_season(
        ...
        for attempt in range(self.config.max_resampling_attempts):
            ...
            # 仅对这个赛季重新采样（启用缓存）
            new_posterior, new_diagnostics, new_samples = temp_sampler.sample_season(
                season, return_samples=True, use_cache=use_cache
            )
