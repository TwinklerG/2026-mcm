...
def plot_improvement_distribution(
    ...
    for patch, left_edge in zip(patches, bins_out[:-1]):
        if left_edge < 0:
            ...
            patch.set_facecolor(COLORS["ptfs"])
    ...
    ax2 = axes[1]
    labels = ["Improved\n(PTFS > Pct)", "Decreased\n(PTFS < Pct)"]
    sizes = [seasons_improved, seasons_total - seasons_improved]
    colors_pie = [COLORS["ptfs"], COLORS["rank"]]
    ...
    ax2.set_title(
        f"Season-Level Improvement Rate\n({seasons_improved}/{seasons_total} seasons)"
    )
