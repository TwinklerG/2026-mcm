...
def plot_judges_save_analysis(results: dict, output_path: Path) -> None:
    ...
    if judges_save_path.exists():
        with open(judges_save_path, encoding="utf-8") as f:
            js_data = json.load(f)
        metrics = {
            "Higher judge score": js_data.get("pct_saved_higher_judge_score", 0.5),
            "Higher fan share": js_data.get("pct_saved_higher_fan_share", 0.905),
            "Higher combined rank": js_data.get("pct_saved_higher_combined", 0.81),
        }
    else:
        metrics = {
            "Higher judge score": 0.50,
            "Higher fan share": 0.905,
            "Higher combined rank": 0.81,
        }
    ...
    ax.set_ylim(0, 1.02)
    ax.set_ylabel("Share of episodes", fontsize=13)
    ax.set_xlabel(None)
    ax.set_title("Judges' Save preference (Seasons 28+)", fontsize=14, fontweight="600", pad=10)
